...
interface PrintableExamRequestProps {
    patientName?: string;
...
export const PrintableExamRequest = React.forwardRef<HTMLDivElement, PrintableExamRequestProps>(
    ({ patientName = "______________________________________________________", clinicalIndication, requestedExams, doctorName = "Dr(a). [Equipe Médica]", doctorCrm = "______" }, ref) => {
        ...
        return (
            <div ref={ref} className="bg-white text-black font-sans leading-normal">
                ...
                */}
                <div className="w-[210mm] min-h-[297mm] p-12 mx-auto flex flex-col relative bg-white border border-transparent">
                    ...
                    {/* ===== INFORMAÇÕES DO PACIENTE & INDICAÇÃO ===== */}
                    <div className="flex bg-[#EBF0F6] px-2 py-1 mb-2">
                        <span className="font-bold text-sm mr-2 min-w-[140px]">PACIENTE:</span>
                        <span className="flex-1 text-sm">{patientName}</span>
                    </div>

                    <div className="flex bg-[#EBF0F6] px-2 py-1 mb-4">
                        <span className="font-bold text-sm mr-2 min-w-[140px]">INDICAÇÃO CLÍNICA:</span>
                        <span className="flex-1 text-sm">{clinicalIndication}</span>
                    </div>

                    <div className="flex bg-[#EBF0F6] px-2 py-1 mb-1">
                        <span className="font-bold text-sm min-w-[140px]">SOLICITO:</span>
                    </div>

                    {/* ===== ÁREA CENTRAL: EXAMES SOLICITADOS ===== */}
                    <div className="flex-1 bg-[#EBF0F6] p-6 mb-8 flex flex-col">
...
PrintableExamRequest.displayName = 'PrintableExamRequest';
